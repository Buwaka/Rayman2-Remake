...
void ParticleEmitterComponent::SetDeathTimer(float time, const GameContext& gameContext)
{
	m_Timer = gameContext.pGameTime->GetTotal() + time;
	m_DeathTimer = true;
}
...
void ParticleEmitterComponent::Update(const GameContext& gameContext)
{
	...
	for (Particle* particles : m_Particles)
	{
		...
		{
			particles->Init(GetTransform()->GetPosition(),true);
			pBuffer[m_ActiveParticles] = particles->GetVertexInfo();
			m_ActiveParticles++;
			m_LastParticleInit = 0.0f;
		}
