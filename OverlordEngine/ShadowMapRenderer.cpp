...
ShadowMapRenderer::ShadowMapRenderer(std::wstring EffectPath)
{
...
void ShadowMapRenderer::Initialize(const GameContext& gameContext)
{
	...
	m_pShadowMat = new ShadowMapMaterial(m_EffectPath);
	...
	desc.IsValid();
	if (FAILED(m_pShadowRT->Create(desc))) Logger::LogError(L"ShadowMapRenderer::Initialize > Failed to create RenderTarget\n");
...
void ShadowMapRenderer::SetLight(DirectX::XMFLOAT3 position, DirectX::XMFLOAT3 direction)
{
	...
	//DirectX::XMVECTOR pos = DirectX::XMLoadFloat3(&m_LightPosition);
	//DirectX::XMVECTOR lookat = DirectX::XMVectorAdd(DirectX::XMLoadFloat3(&m_LightDirection), pos);
	//auto up = DirectX::XMFLOAT3(0, 1, 0);
	//DirectX::XMVECTOR upVec = DirectX::XMLoadFloat3(&up);
	//DirectX::XMStoreFloat4x4(&m_LightVP, DirectX::XMMatrixLookAtLH(pos, lookat, upVec));
	...
	auto proj = DirectX::XMMatrixOrthographicLH(vwidth, vheight, 0.1f, 400.0f); //Near & Far
