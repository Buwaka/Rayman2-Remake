...
void ControllerComponent::Initialize(const GameContext&)
{
	...
	desc.position = physx::PxExtendedVec3(parentPos.x, parentPos.y, parentPos.z);
	desc.userData = this;
...
void ControllerComponent::SetPosition(DirectX::XMFLOAT3 displacement)
{
	if (m_Controller == nullptr)
		Logger::LogError(L"[ControllerComponent] Cannot Move. No controller present");
	else
		m_Controller->setPosition(physx::PxExtendedVec3(displacement.x, displacement.y, displacement.z));
}
