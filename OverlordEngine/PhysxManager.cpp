...
void PhysxManager::Init(void* pDevice)
{
	...
	m_TolScale = physx::PxTolerancesScale();

	m_pPhysics = PxCreatePhysics(PX_PHYSICS_VERSION, *m_pFoundation, m_TolScale, recordMemoryAllocations,
	...
	m_Cooking = PxCreateCooking(PX_PHYSICS_VERSION, *m_pFoundation, physx::PxCookingParams(m_TolScale));
	if (!m_Cooking)
	{
		Logger::LogError(L"Error: PxCreateCooking Failed\n");
		return;
	}
