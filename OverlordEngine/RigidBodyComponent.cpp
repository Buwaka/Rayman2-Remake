...
RigidBodyComponent::RigidBodyComponent(bool isStatic, bool isKinematic) :
	m_pActor(nullptr),
	m_isStatic(isStatic),
...
void RigidBodyComponent::CreateActor()
{
	...
	else
	{
		...
		if (m_ActorFlags)
		{
			if(m_ActorFlags & physx::PxActorFlag::eDISABLE_SIMULATION)
				m_pActor->setActorFlag(physx::PxActorFlag::eDISABLE_SIMULATION, true);
			if (m_ActorFlags & physx::PxActorFlag::eDISABLE_GRAVITY)
				m_pActor->setActorFlag(physx::PxActorFlag::eDISABLE_GRAVITY, true);
			if (m_ActorFlags & physx::PxActorFlag::eSEND_SLEEP_NOTIFIES)
				m_pActor->setActorFlag(physx::PxActorFlag::eSEND_SLEEP_NOTIFIES, true);
			if (m_ActorFlags & physx::PxActorFlag::eVISUALIZATION)
				m_pActor->setActorFlag(physx::PxActorFlag::eVISUALIZATION, true);
...
DirectX::XMFLOAT3 RigidBodyComponent::GetPosition() const
{
	if (m_pActor != nullptr)
	{
		...
		else if (reinterpret_cast<physx::PxRigidDynamic*>(m_pActor)->getKinematicTarget(pose)) {}
...
void RigidBodyComponent::SetActorFlag(physx::PxActorFlag::Enum flag, bool value)
{
	if (m_pActor)
	{
		m_pActor->setActorFlag(flag, value);
	}
	else
	{
		if (value)
