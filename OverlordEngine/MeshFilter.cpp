...
DirectX::XMUINT4 MeshFilter::m_DefaultUInt4 = DirectX::XMUINT4(0, 0,0,0);
...
MeshFilter::MeshFilter():
			...
			m_BlendIndices(std::vector<DirectX::XMUINT4>()),
	...
	for (UINT i = 0; i < m_VertexCount; ++i)
	{
		for (UINT j = 0; j < inputLayoutDescriptions.size(); ++j)
		{
			...
			case ILSemantic::BLENDINDICES:
				memcpy(pDataLocation, HasElement(ilDescription.SemanticType) ? &m_BlendIndices[i] : &m_DefaultUInt4, ilDescription.Offset);
				break;
