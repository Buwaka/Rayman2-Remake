...
float EvaluateShadowMap(float4 lpos)
{
    ...
    //float shadowMapDepth = gShadowMap.Sample(samAnisotropic, lpos.xy).r;
...
float4 PS(VS_OUTPUT input) : SV_TARGET
{
	...
	float4 diffuseColor = gDiffuseMap.Sample( samAnisotropic,input.texCoord + gPanning );
    ...
    return float4(color_rgb * shadowValue * color_a, color_a);
