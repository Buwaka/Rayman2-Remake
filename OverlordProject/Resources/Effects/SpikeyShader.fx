...
VS_DATA MainVS(VS_DATA vsData)
{
 ...
 //   GS_DATA temp = (GS_DATA) 0;
	//temp.Position = float4(vsData.Position,1); // mul(float4(vsData.Position, 1), m_MatrixWorldViewProj);
	//temp.Normal = vsData.Normal; // mul(vsData.Normal, (float3x3) m_MatrixWorld);
...
// GEOMETRY SHADER *
//******************
void CalculateWVP(inout GS_DATA vertex)
{
	vertex.Position = mul(vertex.Position, m_MatrixWorldViewProj);
	vertex.Normal = mul(vertex.Normal, (float3x3) m_MatrixWorld);
}
...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	top = basePoint + gSpikeHeight * spikeNormal;
	//Step 4. Calculate The Left And Right Positions
	float3 spikeDirection = (vertices[0].Position - vertices[2].Position) * 0.1f;
	left = basePoint - spikeDirection;
	right = basePoint + spikeDirection;
	//Step 5. Calculate The Normal of the spike
	float3 leftNormal = cross(left, top);
	float3 rightNormal = cross(right, top);
    ...
    CreateVertex(triStream, left, leftNormal, float2(0, 0));
    CreateVertex(triStream, right, rightNormal, float2(0, 0));
}
