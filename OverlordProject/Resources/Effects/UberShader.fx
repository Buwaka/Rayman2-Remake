...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    float specularStrength = saturate(dot(-viewDirection, reflected));
    specularStrength = pow(specularStrength, gShininess);
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	if(gUseFresnelFalloff)
	{
		...
		environmentColor *= fresnel * fresnelMask;
	}
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	{
		//index refraction
		// float cosi = clamp(-1, 1, dot(viewDirection, normal)); 
		// float3 N = normal;
		// float etai = 1, etat = gRefractionIndex; 
		// if (cosi < 0) { cosi = -cosi; } else { etai = etat; etat = 1; N= -N; } 
		// float eta = etai / etat; 
		// float k = 1 - eta * eta * (1 - cosi * cosi);
		// k = k < 0 ? 0 : eta * viewDirection + (eta * cosi - sqrt(k)) * normal; 

   		float3 reflected = reflect(viewDirection, normal);
		reflected = lerp(viewDirection, reflected, gRefractionIndex) * gReflectionStrength;
